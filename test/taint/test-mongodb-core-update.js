...
(() => {
  ...
  const update = mod.Server.prototype.update;

  // Server.prototype.update
  const param0 = {};
  assert.deepStrictEqual(update('', { 'q': param0 }), ['', { 'q': {} }]);

  const param1 = {};
  param1.foo = '123';
  assert.deepStrictEqual(update('', { 'q': param1 }),
                         ['', { 'q': { 'foo': '123' } }]);

  const param2 = {};
  param2['foo'.setTaint('bar')] = '123';
  assert.deepStrictEqual(update('', { 'q': param2 }),
                         ['', { 'q': { 'foo': '123' } }]);
  ...
  assert.deepStrictEqual(update('', { 'q': param3 }),
                         ['', { 'q': { 'bar': { '$ne': '123' } } }]);
  ...
  assert.deepStrictEqual(update('', { 'q': param4 }),
                         ['', { 'q': { 'bar': '123' } }]);
  ...
  assert.deepStrictEqual(update('', { 'q': param5 }),
                         ['', { 'q': { 'foo': '123', 'bar': '123' } }]);
  ...
  assert.deepStrictEqual(update('', { 'q': param6 }),
                         ['', { 'q': { 'foo': '123', 'bar': '123' } }]);

  // Selector without property
  const selector5 = {};
  selector5['$lt'.setTaint('bar')] = '123';
  assert.deepStrictEqual(update('', { 'q': selector5 }),
                         ['', { 'q': '123' }]);
